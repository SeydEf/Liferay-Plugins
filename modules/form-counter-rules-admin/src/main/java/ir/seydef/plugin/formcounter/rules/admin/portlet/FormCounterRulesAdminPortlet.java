...
public class FormCounterRulesAdminPortlet extends MVCPortlet {
	...
	public void addRule(
		...
		String logicOperator = ParamUtil.getString(
			actionRequest, "logicOperator", "AND");
		...
		int conditionCount = ParamUtil.getInteger(
			actionRequest, "conditionCount");

		String namespace = actionResponse.getNamespace();

		String conditionCountParam = namespace + "conditionCount";

		int conditionCountWithNS = ParamUtil.getInteger(
			actionRequest, conditionCountParam, -1);
		...
		for (int i = 0; i < conditionCount; i++) {
			...
			String operator = ParamUtil.getString(
				actionRequest, "operator" + i);
			String reference = ParamUtil.getString(
				actionRequest, "reference" + i);
		...
		try {
			_formCounterRuleLocalService.addFormCounterRule(
				ruleName, description, ruleModel.toJSONString(), active,
				serviceContext);

			SessionMessages.add(actionRequest, "ruleAddedSuccessfully");
		}
		catch (Exception exception) {
			_log.error("Error adding rule", exception);
			SessionErrors.add(actionRequest, "errorAddingRule");
		}
	}
	...
	public void deleteRule(
		...
		if (ruleId <= 0) {
			SessionErrors.add(actionRequest, "errorInvalidRuleId");
		...
		try {
			...
			SessionMessages.add(actionRequest, "ruleDeletedSuccessfully");
		}
		catch (Exception exception) {
			_log.error("Error deleting rule", exception);
			SessionErrors.add(actionRequest, "errorDeletingRule");
		}
	}
	...
	public void render(
		...
		try {
			List<FormCounterRule> rules =
				_formCounterRuleLocalService.getFormCounterRules(-1, -1);
			...
			if (ruleId > 0) {
				FormCounterRule rule =
					_formCounterRuleLocalService.fetchFormCounterRule(ruleId);

				if (rule != null) {
					renderRequest.setAttribute("rule", rule);

					Rule ruleModel = Rule.fromJSONString(
			...
			SessionErrors.add(renderRequest, "errorLoadingRules");
		}
	...
	public void updateRule(
		...
		try {
			FormCounterRule rule =
				_formCounterRuleLocalService.getFormCounterRule(ruleId);
			...
			SessionMessages.add(actionRequest, "ruleUpdatedSuccessfully");
		}
		catch (Exception exception) {
			_log.error("Error updating rule", exception);
			SessionErrors.add(actionRequest, "errorUpdatingRule");
		}
	}

	private static final Log _log = LogFactoryUtil.getLog(
		FormCounterRulesAdminPortlet.class);
